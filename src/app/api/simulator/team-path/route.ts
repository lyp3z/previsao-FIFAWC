...
import { simulateTeamPath } from '@/modules/simulator/service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.teamId) return fail('teamId is required', 400);

    const data = await simulateTeamPath(body.teamId, body.overrides, DEFAULT_COMPETITION_ID);
    return ok(data);
  } catch (error) {
